...
//  //Case 1 when the level is below 2
//  if( n < 2){
...
//     if( h == 1 ){
//       output_terminals[1].put(std::make_pair(context.max_height, std::make_pair(n + 1, 2 * l)), Node(node.first + 1, 2 * node.second, k, s0, Vector(), false));
//       output_terminals[1].put(std::make_pair(context.max_height, std::make_pair(n + 1, 2 * l +1)), Node(node.first + 1, 2 * node.second + 1, k, s1, Vector(), false));
//     }
//     //Else only put the items
//     else{
//       output_terminals[1].put(h-1, std::make_pair(h-1, std::make_pair(n + 1, 2 * l)), Node(node.first + 1, 2 * node.second, k, s0, Vector(), false));
//       output_terminals[1].put(h-1, std::make_pair(h-1, std::make_pair(n + 1, 2 * l +1)), Node(node.first + 1, 2 * node.second + 1, k, s1, Vector(), false));
//     }
//   }
...
int main(int argc, char *argv[]) {
   ...
   // obj.projecttag.put(std::make_pair(height, make_pair(0, 0)));
   // add_obj.wait();
